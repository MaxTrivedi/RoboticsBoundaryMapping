...
ISR( INT6_vect ) {
  ...
  boolean e_right_B = digitalRead( ENCODER_0_B_PIN ); // normal B state
  boolean e_right_A = digitalRead( ENCODER_0_A_PIN ); // XOR(AB)
 
  // Software XOR (^) logically infers
  // the true value of A given the state of B
  e_right_A = e_right_A ^ e_right_B;
  ...
  // State:  new B   new A   old B   old A
    state_e_right = state_e_right | ( e_right_B  << 3 );
    state_e_right = state_e_right | ( e_right_A  << 2 );

    // Handle which transition we have registered.
    // Complete this if statement as necessary.
    // Refer to tlabsheethe . 
    if( state_e_right == 0 ) {
...
// rotation must have occured.
ISR( PCINT0_vect ) {
    boolean e_left_B = PINE & (1<<PINE2);
    //boolean e_left_B = PINE & B00000100;  // Does same as above.

    // Standard read from the other pin.
    boolean e_left_A = digitalRead( ENCODER_1_A_PIN ); // 26 the same as A8

    e_left_A = e_left_A ^ e_left_B;

    // State :  (bit3)  (bit2)  (bit1)  (bit0)
    // State :  New A,  New B,  Old A,  Old B.
    state_e_left = state_e_left | ( e_left_B  << 3 );
    state_e_left = state_e_left | ( e_left_A  << 2 );
...
void setupEncoder0() 
{
    ...
    // initialise the recorded state of e_right encoder.
    state_e_right = 0;

    // Get initial state of encoder pins A + B
    boolean e_right_A = digitalRead( ENCODER_0_A_PIN );
    boolean e_right_B = digitalRead( ENCODER_0_B_PIN );
    e_right_A = e_right_A ^ e_right_B;

    // Shift values into correct place in state.
    // Bits 1 and 0  are prior states.
    state_e_right = state_e_right | ( e_right_B << 1 );
    state_e_right = state_e_right | ( e_right_A << 0 );
...
void setupEncoder1() 
{
    ...
    // initialise the recorded state of e_left encoder.
    state_e_left = 0;
    ...
    // Shift values into correct place in state.
    // Bits 1 and 0  are prior states.
    state_e_left = state_e_left | ( e_left_B << 1 );
    state_e_left = state_e_left | ( e_left_A << 0 );
